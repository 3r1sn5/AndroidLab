            ...
            new OkHttpClient().newCall(request).enqueue(new Callback()
            {
                ...
                public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException
                {
                    if(response.code() == 200)
                    {
                        ...
                        {
                            items[i] = "The train is arriving at : " + data.result.results[i].Station + "\n" + "Terminal Station : " + data.result.results[i].Destination + "\n";
                        }
                        runOnUiThread(() ->
                        {
                            new AlertDialog.Builder(MainActivity.this).setTitle("Taipei MRT Station").setItems(items, null).show();
                        });
                    }
